...
export const CertificateDialogContent = ({handleDownloadClick}) => {
  return (
    <div className="flex flex-col justify-center items-center">
      ...
      <h1 className="text-6xl font-semibold mx-[10px] mt-16">Congratulations 🎉</h1>
      <p className="text-lg-line-height text-gray mt-4">People like you make the world a better place!</p>
      ...
      <div className="w-full mx-14 pt-11 grid grid-cols-2 grid-rows-2 gap-4">
        <CertificateDialogDetail title="Transaction Hash" value="0x0008c5752257..." />
      ...
      <div className="w-full flex justify-between items-center mt-8 mb-14">
        <CertificateDialogButton text={"Download Certificate"} icon={download} onClick={handleDownloadClick} />
...
const CertificateDialogButton = ({text, onClick, icon}) => {
  return (
    <Button onClick={onClick} className="text-dark px-0" variant="ghost" size="md">
...
const FooterCirclers = () => {
  return (
    <div className="absolute flex -bottom-3">
      {Array.from({length: 13}).map((val, index) => <div key={index} className={`bg-dim w-6 h-6 rounded-full mr-2.5`} />)}
    </div>
  );
}
