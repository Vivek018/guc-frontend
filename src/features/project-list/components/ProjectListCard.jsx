  ...
  return (
    <div className="w-[330px] h-[544px] bg-white shadow-card font-dm-sans rounded-b-md">
      <div className="w-full h-52 overflow-hidden z-10">
        <img className="h-full z-0 object-fill rounded-t-md" src={thumbnail} alt="" />
      </div>
      <div className="flex flex-col px-5 pt-4 pb-6 justify-start">
        ...
        <p className="mt-6 text-gray text-lg">Last donation {formatted_donation_time}</p>
        <ProgressBar className="mt-3" value={33} />
        <p className="mt-3 text-lg text-gray">
          <span className="font-bold font-poppins mr-1 text-black">${donation_amount_raised}</span>
          <span>
              raised of ${donation_total_amount}
          </span>
        </p>
        <div className="mt-6 flex gap-2">
          ...
          <Button variant="outline" size="md">Donate now</Button>
        </div>  
      </div>
    </div>
  );
